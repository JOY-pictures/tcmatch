...
import com.devlink.devlink.bot.handlers.CallbackHandler;
...
public class DevLinkBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().hasText() &&
            ...
            // 1. Команда /start - точка входа
            handleStartCommand(update);
        }
        else if (update.hasCallbackQuery()) {
            handleCallbackQuery(update.getCallbackQuery());
        } else {
            handleUnknownInput(update);
        }
    }
    ...
    private void handleStartCommand(Update update) {
        ...
        String welcomeText = callbackHandler.getWelcomeText(chatId, userName);
        InlineKeyboardMarkup keyboard = keyboardFactory.getKeyboardForUser(chatId);

        sendInlineMessage(chatId, welcomeText, keyboard);
        log.info("🎯 Start command from {}: {}", chatId, userName);
    }
