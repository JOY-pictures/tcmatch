...
public class EditApplicationFieldCommand implements Command {
    ...
    public boolean canHandle(String actionType, String action) {
        return "application".equals(actionType) && "edit_field".equals(action);
    }
    ...
    public void execute(CommandContext context) {
        try {
            ...
            state.moveToEditField(context.getParameter());
            ...
            showCurrentStep(chatId, project, messageId);

        } catch (Exception e) {
            log.error("❌ Ошибка редактирования поля отклика: {}", e.getMessage());
            botExecutor.sendTemporaryErrorMessage(context.getChatId(), "Ошибка редактирования отклика", 5);
        }
    }
